...
const iluminate = (iluminated, direction: Direction, irow: number, icol: number) => {
...
const totalEnergy = (iluminated) =>
    iluminated.map(line => line.filter(cell => cell != 0).length).reduce((a, b) => a + b, 0);


const bestIlumination = () => {
    let best = 0;
    for (let i = 0; i < map.length; i++) {
        let iluminated = Array.from({ length: map.length }, () => Array.from({ length: map[0].length }, () => 0));
        iluminate(iluminated, Direction.Right, i, 0);
        best = Math.max(best, totalEnergy(iluminated));
        iluminated = Array.from({ length: map.length }, () => Array.from({ length: map[0].length }, () => 0));
        iluminate(iluminated, Direction.Left, i, map[0].length - 1);
        best = Math.max(best, totalEnergy(iluminated));
    }

    for (let i = 0; i < map[0].length; i++) {
        let iluminated = Array.from({ length: map.length }, () => Array.from({ length: map[0].length }, () => 0));
        iluminate(iluminated, Direction.Down, 0, i);
        best = Math.max(best, totalEnergy(iluminated));
        iluminated = Array.from({ length: map.length }, () => Array.from({ length: map[0].length }, () => 0));
        iluminate(iluminated, Direction.Up, i, map.length - 1);
        best = Math.max(best, totalEnergy(iluminated));
    }
    return best;
};

console.table(bestIlumination());
